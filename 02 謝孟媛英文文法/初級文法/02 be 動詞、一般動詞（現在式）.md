...
- **be 動詞**（am／are／is）→ 否定、疑問**靠自己**
- **一般動詞** → 否定、疑問**借助動詞 do／does**，動詞回原形

## 🅱️ be 動詞 am／are／is

- 表**狀態**或**存在**
  - 是：We **are** happy.／在：She **is** in America.
- 用哪個看**主詞人稱**：I→am、you／複數→are、三單→is
  - I **am** a boy.（I 永遠大寫）／He **is** my student.
- 否定：主詞＋be＋**not**
  - He **is not**（= isn't ＝ He's not）a good baseball player.
    - ⚠️ I am not 只有 **I'm not**（無 amn't）
- 疑問：**be 移到句首**
  - That is his camera. → **Is** that his camera?
- 簡答：用 be 問、用 be 答；**主詞必用代名詞**
  - Is that man your math teacher? → Yes, **he** is.／No, **he's** not.

## 🟢 一般動詞

- 日常**具體**與**抽象**動作：eat、walk／like、think
- 肯定句：主詞三單 → 動詞加 **s／es**
  - He **likes** dogs.（I／You／They like dogs.）
  - 規則：大部分＋s；z・o・s・x・sh・ch＋es（goes、watches）；子音＋y→ies（studies）；母音＋y＋s（says）
    - ⚠️ 特殊：have → **has**（He **has** a lot of money.）
  - 💬 判斷訣竅：能用 he／she／it 代替的主詞都算三單（The boy＝He）
- 否定：主詞＋**do／does＋not＋原形**（不可直接加 not）
  - The twin brothers **don't go** to school by bus.
  - Sam **doesn't have** dinner at the restaurant.
    - 助動詞已表時間 → 動詞**回原形**
- 疑問：**Do／Does＋主詞＋原形**（不可把動詞移句首）
  - **Do** you **visit** your grandmother on Sundays?（on＋星期幾s＝每逢）
  - **Does** he **come** from England?
- 簡答：用助動詞答、**主詞必用代名詞**；do／does 可**代替前面的動作**
  - Does the little boy go to school? → Yes, he **does**.（does＝goes to school）

## 📊 兩條路合看

- be：He **is**／**isn't** my boyfriend.／**Is** she beautiful?
- 一般：He **likes**／**doesn't like** dogs.／**Does** she **love** tennis?
  - 一句話：be 什麼都自己來；一般動詞找 do／does、動詞回原形

## ⚠️ 高頻錯誤

- ❌ Does she loves? → ✅ Does she **love**?（助動詞後回原形）
- ❌ He like dogs → ✅ He **likes** dogs（三單漏 s 最常見）
- 錯題：❌ trys → ✅ **tries**；❌ does not watche → ✅ does not **watch**（+es 整組拔掉）
```
...
> **答句注意點**
> - 用助動詞問，就用助動詞回答。
> - **答句中的主詞必須用代名詞**（the little boy → he）——與 be 動詞簡答同一條規則。
...
> 由 [[02 謝孟媛英文文法/診斷/初級 02 be 動詞、一般動詞（現在式） 診斷卷|初級 02 診斷卷]] 挖出、**實際答錯**的三條（規則見 [[使用說明]] 4-2）。已進 [[易錯點總表]] 在役表跑間隔。本章主幹（be／一般動詞分流、do／does、動詞回原形、三單加 s 規則）18 題全數過關——錯的都在主幹之外的縫隙。

1. **並列動詞：主詞管到 and 後面的每一個動詞**——三單 s 不是只加第一個就好。
   - ❌ She studies English and **go** to bed early. → ✅ …and **goes** to bed early.
2. **否定句的並列動詞用 or 連接**（兩件都否定）；用 and 語意會偏向「不是『兩件都做』」。
   - ❌ She doesn't study English **and** go to bed early. → ✅ …study English **or** go to bed early.
   - （does not 之後兩個動詞都回原形，這一步沒有錯。）
3. **every day（每天，副詞、兩個字）≠ everyday（日常的，形容詞）**——同一份卷連中兩次。
   - ❌ He does not run **erveyday**.／❌ …drinks a lot of water **everyday**. → ✅ **every day**
   - 對照：**everyday** life（形容詞修飾名詞時才連寫）。

> [!NOTE]
> 同份診斷卷另挖出幾條不屬本章、已升級到 [[易錯點總表]]：**單數可數名詞前要有冠詞**（❌ He is not handsome boy → ✅ …**a** handsome boy）與**「爸媽」是兩個人、parents 要複數**（❌ your parent → ✅ your parents），兩條歸 [[01 名詞、冠詞]]；**審題與完整作答**（cats 看成 cars、漏譯「新來的／每天」、沒按題目要求寫理由）這條作答習慣；句中大寫（Are **The** children）則併入初 01 已在役的大小寫／拼字滑手條。
