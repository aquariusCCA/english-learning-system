...
> [!WARNING]
> **某些動詞不用進行式——因為它們是「狀態」，不是「動作」。**
> 進行式（be + V-ing）像把鏡頭推近，拍一個「正在進行、下一秒可停」的**動作**；
> 下面這些詞描述的卻是**一直如此的狀態或本能知覺**，沒有「進行到一半」的畫面可拍：
> - **感官**動詞：see、hear、smell…（眼睛睜著就看見，不是你一秒一秒在「做」的事）
> - **情感**動詞：love、like…（感情是背景狀態；**標語例外** I'm loving it）
> - **狀態**動詞：have（擁有）、know…（擁有／知道是穩定狀態，不會開開關關）
> - 例：我正在看樹上那隻鳥 → ❌ I'm **seeing** the bird → ✅ I'm **looking at** the bird.
>   （see 是自動知覺、不會下一秒看不到；改用主動動作 look at 才拍得到「正在做」）
