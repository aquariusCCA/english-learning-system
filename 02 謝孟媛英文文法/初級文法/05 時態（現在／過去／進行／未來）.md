...
- ⚠️ 某些動詞不可用進行式（測試：**下一秒能停嗎？**）：感官 see・hear・smell（器官基本功能）／情感 love・like（標語 I'm loving it 除外）／其他 have（擁有，持續）・know
...
進行式是**暫時性的描述**——只說「此刻正在發生、持續中」，不承諾會一直持續，也不承諾未來不變。所以一個動詞配不配進行式，用一個測試就能判斷：

> [!IMPORTANT]
> **測試：這個動作能不能「這一秒在做、下一秒停止」？**
> - eat **能**——我可以這一秒正在吃飯、下一秒就停止 → We're eating ✅
> - see **不能**——我上一秒正在看，難道下一秒就瞎了嗎？當然不是 → I'm seeing ❌
>
> 能停的是「動作」，可用進行式；停不了的是狀態／功能，不用進行式。

三類過不了測試的動詞（皆為來源整理）：

| 類 | 動詞 | 為什麼過不了測試 |
| --- | --- | --- |
| **感官** | see、hear、smell 等 | 指的是**器官的基本功能**——上一秒在看，下一秒不會瞎 |
| **情感** | love、like 等 | 你總不會上一秒愛媽媽、下一秒就不愛了吧？——**但「標語」除外**：麥當勞 **I'm loving it**（漢堡可以上一秒愛、下一秒不愛，玩的正是這個反差），所以這類動詞**通常只在標語**加 V-ing |
| **其他** | have（擁有）、know 等 | have＝**擁有**，是持續性的——爸爸買錶給你，這一秒擁有、下一秒不會就失去；到弄壞、搞丟之前都擁有 |

**例：我正在看樹上的那隻鳥。**
- ❌ I'm **seeing** the bird in the tree.——see 是眼睛的基本功能，**停不了** → 不能進行。
- ✅ I'm **looking at** the bird in the tree.——**look at ＝ 注視**：上一秒盯著看、下一秒可以不盯，**能停** → 可以進行。

> 💬 診斷卷考過的 **want** 同族：「想要」也停不了（沒有下一秒就不想的過程），配 right now 也用 I **want**，不用 I'm wanting。
