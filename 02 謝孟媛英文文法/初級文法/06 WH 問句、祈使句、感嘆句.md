...
> 由 [[02 謝孟媛英文文法/診斷/初級 06 WH 問句、祈使句、感嘆句 診斷卷|初級 06 診斷卷]] 挖出、**實際答錯**的兩條（規則見 [[使用說明]] 4-2）。已進 [[易錯點總表]] 在役表跑間隔。

1. **疑問詞當補語時，be 跟後面的「真主詞」一致；「視為單數」只適用於疑問詞當主詞**。（Q4）
   - ❌ Whose **is** these toys? → ✅ Whose **are** these toys?
   - 還原直述句：These toys are **whose**——whose 是**補語**、主詞是 **these toys**（複數）→ be 用 are。「疑問詞視為單數」是疑問詞**自己當主詞**時的規則（Who **is** cooking?），誤套到補語就翻車。兩型並排記：主詞 → Who **is** cooking?；補語 → Whose **are** these toys?
2. **How 後面接形容詞還是副詞：把感嘆句還原成直述句，看它修飾誰**。（Q18）
   - ❌ How **honestly** you are! → ✅ How **honest** you are!
   - 還原：You are **honest**——be 動詞後的**補語用形容詞**，副詞不能當補語。用副詞的是修飾**一般動詞**的情況（How **fast** he swims!——fast 修飾 swims）。從 fast 那型過度類推到 be 句就錯了。

> **同卷在役舊條又中四條**（證據已補回 [[易錯點總表]]）：
> - **單數可數名詞不能裸奔**（Q14 一句兩處）：eating **snack** → **snacks**（泛指用複數）；listen to **teacher** → **the teacher**。連對歸零。
> - **專有名詞與 the**（Q6，反向踩）：**the** Mt. Everest → **Mt. Everest**——單座山不加 the；要加的是省略型（the Yangtze）與山脈（the Alps）。條目已擴充、連對歸零。
> - **大小寫**（一份卷五處）：句中 **K**itchen／**L**et's ×2／Please **S**it／Please **S**top 全該小寫——大寫只給句首與專有名詞。（另：句尾要用半形 `?`，不用全形「？」。）
> - **審題與完整作答**（Q1・Q12）：「為什麼不用 are」「各是對誰說的」——兩處都是兩問只答一問。
>
> **另附一筆不入表的觀察**：Q17 `Which one of two hats`——特定範圍「那兩頂」要加 the（of **the** two hats），且提示要求用疑問形容詞＋名詞（**Which hat** do you like best?）；意思到了、未達錯點層級。
