...
> [!WARNING]
> **練習卷錯題回填（2026-07-21，[[初級07 比較 練習卷]]）　💬 AI 補充**
> ⚠️ **本篇與 04–06 的狀況不同**：那三章是「懂了但落筆漏」，這份卷甲部 10 題只對 3 題、3 題整題空白——**三級變化表尚未背起來**。下列只收觀念錯；字形本身另走專項練習（見末條）。
> - **① 比較級後面不接名詞**：❌ Sam is **smarter student** than any other students ✅ Sam is **smarter than any other student** in the class.
>   - 理由：最高級是「同類中最…的**那一個**」所以帶名詞（the smartest **student**）；改成比較級後是**兩邊**在比，形容詞後面直接接 than。
>   - 搭配數也固定：**any other + 單數**（any other student）／**all the other + 複數**（all the other students）。
> - **② of the two 要加 the**：❌ Ben is **much taller** of the two brothers ✅ Ben is **the taller** of the two brothers.——範圍被限定成兩個，「其中較高的那位」是特定的人，所以比較級**要加 the**（比較級少數加 the 的情況）。
> - **③ 多音節一律 more／most，不可自創 -er**：❌ **expensiver**、❌ **wisier**、❌ most **tall** ✅ **more expensive**、**wiser**、**tallest**。
> - **④ 單複數要跟到底**：❌ No other **students** … **is** smarter ✅ No other **student** … **is**／No other **students** … **are**（no 後接單數複數皆可，但動詞必須配合）。
> - **⑤ 最高級句型裡沒有 than 的位置**：❌ the most touching **than** that I have ever seen ✅ **the most moving movie that** I have ever seen.（形容詞最高級的 the 不可省，後面接名詞再接 that 子句）
> - **⑥ 不規則變化優先於 more／most**：❌ dances **more well** ✅ dances **better**（有不規則形就不走 more）。⚠️ 我在甲 6 剛把 well→better→best 寫對，同卷丁 5 仍寫 more well——**表上會 ≠ 句中會**。
> - **⑦ many／much 的比較級是 more，不是 much**；much 本身是原級（配不可數，many 配可數），兩者共用 more → most。
> - **⑧ latest／last、farther／further 問的是語意分工，不是三級**：latest ＝ 時間上最新（the latest news）／last ＝ 順序上最後（the last page）；farther ＝ 距離更遠／further ＝ 程度更進一步（further information）。
> - 💬 **字形另練**：-er／-est 的雙寫、去 e、子音+y 這組規則與 [[05 時態（現在／過去／進行／未來）]] 的 V-ing **是同一組通則**，已合併出卷 → [[初級 字尾變化通則專項練習卷（05 V-ing ＋ 07 比較級）]]。
