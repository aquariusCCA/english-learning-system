...
- Philippines 的身世正好可當驗證：西班牙文 Las Islas Filipinas（腓力的群島）→ 英文 the Philippine **Islands**（Philippine 當形容詞、主體是 Islands）→ 省略 Islands → the Philippines。這個 the 其實是加給被省略的 Islands 的，連 -s 都是從它繼承來的——正是下一條「省略」的邏輯提前登場。

**③ 普通名詞被省略、the 留下（河流、沙漠等）**——完整說法是 the Nile **River**。這裡要先看清楚**字的順序**：英文名詞片語的**主體（中心語）在最右邊**，左邊的字都是拿來修飾它的。所以 the Nile River 的主體是普通名詞 **River**（講的是一條河），Nile 只是形容「哪一條」——這個 the 跟 the door 的 the 完全是同一個，限定的對象是 River。

這類組合講太多次，River 被省掉偷懶，但骨架沒變，the 指著的還是那個心照不宣的 River——**the 不是加給 Nile 的，是加給被省略的 River 的**：
...
- **驗證法：把名詞放回去，講得通就是這一類**——the Nile River ✔、the Ritz Hotel ✔。放不回去的就不是省略：Mount Everest 不是從 the Everest Mount 省來的（順序就不對，普通名詞在前面），它是下面的「頭銜＋名字」構造。

**對照組：Mount Everest、Lake Superior 明明含普通名詞，卻不加 the**——因為順序反過來了。用剛剛的「主體在最右邊」一量就分明：

| 構造 | 例 | 主體（最右邊那個字） | the |
| --- | --- | --- | --- |
| 普通名詞在**後**（描述型） | the Nile **River**、the Atlantic **Ocean**、the Empire State **Building** | 普通名詞 | 要 |
| 普通名詞在**前**（頭銜型） | **Mount** Everest、**Lake** Superior | 名字 | 不要 |

普通名詞站到名字前面時，身分就從「主體」變成**頭銜**——跟人名前面的 Doctor、Uncle、President 是同一種東西，會被吸收成名字的一部分，而**名字不需要 the**：

- **the** doctor ✔（普通名詞，要 the）／Doctor Smith ✔、the Doctor Smith ✘
- **the** president ✔／President Lincoln ✔、the President Lincoln ✘
- **the** mountain ✔／Mount Everest ✔、the Mount Everest ✘

同一個普通名詞，**位置決定身分**：在後面是主體（要 the），在前面是頭銜（併入名字，不要 the）。

**驗證法：改成 of 結構**——主體被推回最左邊，the 立刻就回來了：
- Mount Everest（不加）→ **the** Mount **of** Olives（橄欖山，主體是 Mount → 加）
- Lake Superior（不加）→ **the** Lake **District**（湖區，主體是 District → 加）
- 同理 the **Gulf** of Mexico、the **Isle** of Man——只要普通名詞當了主體，the 一定跟著出現。

這也解釋了看似矛盾的一條：**山脈（the Alps）要加、單一山峰（Mount Everest）不加**——不是任意規定，是兩種完全不同的構造（一個是複數普通名詞打包，一個是頭銜＋名字）。水域也一樣：Lake Superior 不加、**the** Great **Lakes**（五大湖）要加。

> [!NOTE]
> **但書：這是主流規律，不是百分之百。** 從德文等借來的山名沒有頭銜可掛，被當成「省略型」處理而帶 the：the Matterhorn、the Eiger；the Hague（海牙）則是荷蘭文 Den Haag 的歷史遺留，the 本來就長在名字裡。這類名字直接記，不要硬套規則。
...
### 診斷卷實測錯點（2026-07-22）　💬 AI 補充
> 由 [[02 謝孟媛英文文法/診斷/初級 01 名詞、冠詞 診斷卷|初級 01 診斷卷]] 挖出、**實際答錯**的八條（規則見 [[使用說明]] 4-2）。已進 [[易錯點總表]] 在役表跑間隔。

1. **不可數名詞的「三不」**（同一題根連中三次）——不加 a/an、沒有複數、**泛指時也不加 the**。
   - ❌ two loafs of **breads** → ✅ two loaves of **bread**（變複數的只有容器 loaf→loaves）
   - ❌ **The** honesty is the best policy. → ✅ **Honesty** is…（加了 the 就變成「那份特定的誠實」，與泛指的通則不合）
   - ❌ **an** information → ✅ **some** information／**a piece of** information（借單位，同物質名詞借容器）
2. **a/an 看「發音」不是看拼字**——[j] 開頭的字拼字是母音、發音卻是子音。
   - ❌ **an** European tour、**an** useful tool → ✅ **a** European、**a** useful
   - 對照：an hour、an honest（h 不發音才是真母音開頭）
3. **專有名詞的省略型 the**——「是專有名詞所以不加」是要打掉的直覺；先問主體是名字還是（被省略的）普通名詞。
   - ❌ Yangtze → ✅ **the** Yangtze（＝the Yangtze **River** 省略 River）
   - 驗證法：把名詞放回去講得通 → 就是省略型 → 要 the
4. **不規則複數（要個別記的那一類）**——規則變化熟，倒在這裡。
   - ❌ foots、crisises → ✅ **feet**（改母音）、**crises**（外來語 -is → -es）
5. **複數字尾非 s 時，所有格照原則加 's**。
   - ❌ **woman's** shoes（想講「女鞋」）→ ✅ **women's** shoes
6. **個別所有格：名詞本身也要跟著變複數**——動詞改了、名詞漏改的半套。
   - ❌ Tom's and Jerry's **teacher** are strict. → ✅ …**teachers** are strict.
7. **the ＋ 形容詞＝「～的那群人」，動詞用複數**。
   - ❌ The poor **is** not always happy. → ✅ The poor **are**…
8. **計量片語 of 後面**：可數名詞要複數、物質名詞不動（與第 1 條互補）。
   - ❌ a handful of mung **bean** → ✅ a handful of mung **beans**
   - 對照：a handful of **sand**（物質，不動）

> [!NOTE]
> 同份診斷卷另挖出三條不屬本章、已升級到 [[易錯點總表]]：**過去時間副詞要配過去式**（The audience **was** huge last night → `03`／`05` 時態）、**主詞動詞一致＋is broke → is broken**（`13` 動詞／`16` 被動語態）、以及**專有名詞首字母大寫／拼字滑手**（Mount **F**uji、**an** armful）這條作答習慣。
