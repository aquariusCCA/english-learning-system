...
- it 的特殊用法
  ...
  - 假主詞（代替一件事：to V／that 子句／V-ing 放後面說明）：**It** is difficult to learn Spanish.
...
- this／that：近用 this(these)、遠用 that(those)
  ...
  - 電話 Who is **this**?（較近）／**Who was that** on the telephone?（較遠）
...
- **當假主詞——代替「一件事」**：it 當假主詞指的是什麼事情，要找**不定詞（to + V）**或**連接詞 that 所引導的子句**（找不到不定詞或 that，就找 **V-ing** 引導的句子）——真主詞可用 it 代替、把那件事**放到後面說明**：
  - It is difficult **to learn Spanish**.（西班牙文很難學。）→ it 代替 to learn Spanish（to＋原形動詞叫不定詞）
  - It's important **that you should tell the truth**.（你該說實話，這很重要。）→ it 代替 that 子句；有主詞＋動詞 → 用 that（用不定詞會變 to tell the truth，you、should 就不見了）
    - that 當連接詞時**不翻成「這個／那個」**，純粹用來連接——句子的基本結構是一個主詞＋一個動詞，出現兩個主詞、兩個動詞時一定要有連接詞。
  - It's no use **telling him about it**.（告訴他這件事是沒用的。）→ it 代替 telling him about it
    - 不定詞為首的主詞可用假主詞 it 代替、事件放到後面：**To tell him about it** is no use. ＝ It's no use **to tell** him about it.
    - 動名詞當主詞同理：**Telling him about it** is no use. ＝ It's no use **telling** him about it.
...
> [!NOTE]
> **「雙重所有格」這名字怎麼來？　💬 AI 補充**
> 此為 AI 補充、非來源內容。從「想講什麼」倒推，這名字就好懂了：
>
> **① 先看麻煩在哪**：想說「我的**一位**老朋友」，得同時用上 a（一位）和 my（我的）。但冠詞、所有格、指示形容詞這些**限定詞，一個名詞前只能站一個**，不能疊著用——❌ **a my** old friend。
>
> **② 解法**：a 留在原位，把「我的」**搬到名詞後面**、用 of 接上 → ✅ an old friend **of mine**。
> 搬過去的是 **mine** 不是 my——of 後面得站一個**能單獨存在**的字，而上一段剛說過：所有格（my）不能單獨存在，所有代名詞（mine）可以。
>
> **③ 名字由來**：搬完之後，「…的」被標了**兩次**——**of** 本身就有「…的」的意思，**mine** 又自帶所有格身分。**of ＋ mine ＝ 兩個「所有」疊在一起，所以叫「雙重」所有格。**
>
> **④ 名詞版同理**：an old friend **of John's**——of 一次、's 一次，一樣是雙重。
...
- **遠近**：離說話者**近**用 this(these)、**遠**用 that(those)。
  ...
  - 電話用語：**Who is this?**（你是誰？正在跟你講電話的，較近）／**Who was that** on the telephone?（電話上那人是誰？較遠）。（敲門的 Who is **it**?、當面的 Who are **you**? 用的是人稱代名詞，見上方「👤 人稱代名詞」一節）
...
> [!NOTE]
> **交代一下：①②的分組跟講義不一樣　💬 AI 補充**
> 講義原本把上面**所有** so 的例句放在同一個標題「代替前面的字句，當受詞或補語」底下；本篇把它們拆成①②，因為兩類的 so 做的事不同：
> - `I hope **so**`：so 站在動詞 hope 後面、當它的**受詞**（＝that it will be fine tomorrow）→ 真的是「受詞／補語」用法（①）。
> - `**so** can I`：so 代替的是前面整個**述語**（＝主詞後面「做什麼」的那一段：can play the violin），還把句子倒裝成「so＋助動詞＋主詞」——它不是任何動詞的受詞，是**副詞**用法（②）。
> 例句與括號註解都出自講義、沒有新造；拆組是教學型重構的編排調整，特此留記供回源核對。
...
### other / another
- **other**：其他人事物。**單獨一個 other 只能當形容詞**（後面要接名詞：other people），不能自己站著；**加 the 或變複數**才是代名詞——**the other**（兩個中的另一個）、**others**（other 的複數，泛指其他）、**the others**（剩下的那些）。（用本節判準驗：例句裡 others／the others 都單獨當主詞。）
- **another**（= an + other）：不特定的「另一個」，**無複數**。

![other／another 六種情境示意圖（來源附圖）：one–the other（2 個）、one–the others（1＋其餘）、one–another（3+ 不指定）、one–another–the other（3 個逐一點）、some–the others（有限定）、some–others（無限定）](images/04%20other-another.png)
...
- **none**：人或物皆可，可數／不可數皆可（當主詞時單複數動詞都行）。
  ...
  - 💬 **AI 補充**（講義只在 neither 條目講這條通則，沒點到 none）：none 跟 neither 一樣**本身是否定字**（None of…＝「…沒有任何一個」），**不可再與 not 同用**——❌ I **don't** know **none** of them → ✅ I know **none** of them／I **don't** know **any** of them（none ≈ **not ＋ any**）。
- **several**：數個，只用於**可數複數**。
...
> 由 [[02 謝孟媛英文文法/診斷/初級 04 代名詞 診斷卷|初級 04 診斷卷]] 挖出、**實際答錯**的五條（規則見 [[使用說明]] 4-2）。已進 [[易錯點總表]] 在役表跑間隔。

1. **代替前文那個特定的東西，用受格 it；his／hers 是「他的東西」，不是「它」**。
   - ❌ My sister borrowed his bike and returned **his** to him. → ✅ returned **it** to him
   - 同一句裡兩個洞不同格：「他的腳踏車」＝所有格 **his** ＋名詞；「它」＝指回前面那台車、**特定** → 受格 **it**（＝ the bike）。寫 returned his 變成「把他的（東西）還給他」，沒交代還的是什麼。
   - 同卷 Q9（`bought one` vs `it`）判對了——**有提示在考代名詞時判得出，沒提示時就漏判**。
2. **not ＋ all／both ＝「並非全部」（部分否定）；「全部都不」要用 no／none／neither**。（**重犯**，初 03 診斷卷 Q14 同條）
   - ❌ **All** students **do not** like math.（讀成「所有學生都不喜歡」）→ ✅ **Not all** students like math.
   - ❌ I **don't** like **both** of the two shirts（＝並非兩件都喜歡）→ ✅ I like **neither** of the two shirts／I **don't** like **either** of them
   - 判準：**否定詞放哪就否定哪個字**——要否定 all／both，not 就得貼在它前面；要「全部都不」就換否定字（no／none／neither）扛。
   - 同卷 Q11 把「neither 是否定字、不可與 not 同用」的**理由寫對了**，拆掉 not 之後卻補上 both——**規則背得出、選字仍錯**。
3. **another 只用於總數 3 以上；總數 2 的第二個一定是 the other**。
   - ❌ I have two cats, one is black, **another** is white. → ✅ … one is black; **the other** is white.
   - another ＝ an ＋ other ＝「不指定的再一個」；總數只有 2 時，講完一個就只剩唯一那個，**沒有「不指定」的空間** → 必須限定 → the other。
4. **否定改肯定時，時態會從助動詞「掉回」主要動詞**。（跨章：歸 [[03 be 動詞、一般動詞（過去式）]]）
   - ❌ I **didn't** buy none… → I **buy** none of the snacks. → ✅ I **bought** none of the snacks.
   - 拆掉 didn't，did 帶走的過去式沒人扛了，主要動詞得自己變過去式（同理 He didn't go → He **went**）。none 用對了、時態掉了。
5. **假主詞的 it 代替的是「同一句後半」那件事（to V／that 子句），不是前文的某件事**。（措辭於 2026-07-25 回源修正：來源定義本就是「假主詞→代替一件事」，錯點在於沒認出代替的是哪件事、沒答出「假主詞」這個身分）
   - ❌ It is impossible to finish the work in one day. 的 it 只答「表示一件事情」→ ✅ 要指出這是**假主詞**：它代替的那件事就是**同句後面的 to finish the work in one day**（真主詞放後面說明）。
   - 驗證法：把後面那段**搬回句首**看成不成立——To finish the work in one day is impossible. ✓ 成立 → 是假主詞。（對照：`I think **so**` 的 so 代替的是**前文**的事；假主詞的 it 代替的是**同一句後半**。）

> **另附三筆不入表的觀察**（同卷發生、未達錯點層級）：
> - **Q6 沒踩到考點**：「她和她媽媽用同一款手機」寫成 She and her mother use the same cellphone.（文法無誤，但讀得出「共用一支」）；本章要的是 **the same … as**：She uses **the same** phone **as** her mother (does).
> - **Q12 答案對、理由錯**：`If you have any questions…` 判成「any 用於疑問句」——這是 **if 條件句**（句尾是句號）。完整原則：some 用肯定句＋請求/邀請問句；any 用否定、疑問、**條件句**。
> - **Q8 只答一半**：`so do I` 說出「也」的意思，但沒點出 so 代替的是**述語** likes tea、且帶出**倒裝**（so＋助動詞＋主詞），詞性是**副詞**——這正是它與 `I think so`（當受詞）的分界。
