...
5. **假主詞的 it ＝「先頂位子」，不是「代表某一件事」**。
   - ❌ It is impossible to finish the work in one day. 的 it 判成「表示一件事情」→ ✅ **假主詞**：真主詞是後面的 **to finish the work in one day**，因為太長才派 it 去佔主詞位。
   - 驗證法：把後面那段**搬回句首**看成不成立——To finish the work in one day is impossible. ✓ 成立 → 是假主詞。（「代表一件事」的是 `I think **so**` 的 so，代替的是**前文**；假主詞的 it 代替的是**同一句後半**。）
